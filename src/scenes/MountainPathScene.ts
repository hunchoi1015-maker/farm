...
export class MountainPathScene extends Phaser.Scene {
  ...
  constructor() { super({ key: SCENE_KEYS.MOUNTAIN_PATH }); }
  ...
  private getSpawnPos(data?: FromData): { x: number; y: number } {
    ...
    switch (data?.from) {
      case 'beach': return { x: cx, y: 4 * TILE_SIZE };
  ...
  private createExitZones(): void {
    ...
    const portals = [
      { fromKey: 'mountain_path', toKey: 'mountain', target: SCENE_KEYS.MOUNTAIN,
        x: T*6, y: 0,         w: T*8, h: T*2, dir: 'up'   as const },
      { fromKey: 'mountain_path', toKey: 'beach',    target: SCENE_KEYS.BEACH,
