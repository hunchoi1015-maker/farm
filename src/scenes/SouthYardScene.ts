...
//   - tileId = "south_yard:tx:ty" (맵 크기 변경에 무관)
//   - 땅 모양: 반짝이는 이펙트로 표시
...
export class SouthYardScene extends Phaser.Scene {
  ...
  constructor() {
    super({ key: SCENE_KEYS.SOUTH_YARD });
  }
  ...
  create(): void {
    ...
    this.canFarm  = this.gm.gameState.houseLocation === 'south';
    ...
    console.log(`[SouthYardScene] 생성 완료 — 농사 가능: ${this.canFarm}`);
  }
  ...
  private buildBackground(): void {
    ...
    // 집 위치 표시 (남쪽 마당 - 맵 하단)
    this.add.rectangle(20 * TILE_SIZE, 32 * TILE_SIZE, 6 * TILE_SIZE, 4 * TILE_SIZE, 0xb8a090)
      .setOrigin(0, 0).setDepth(1);
    this.add.text(20 * TILE_SIZE + 4, 32 * TILE_SIZE + 4, '남쪽 집', {
  ...
  private createPlayer(): void {
    ...
    const spawnY = 4 * TILE_SIZE;  // 북쪽(마을) 방향에서 진입
  ...
  private createExitZones(): void {
    ...
    const exits = [
      { name: 'exit_north',  x: 0,      y: 0,                    w: worldW, h: T * 2, target: SCENE_KEYS.VILLAGE     },
      { name: 'exit_south',  x: 0,      y: worldW - T * 2,       w: worldW, h: T * 2, target: SCENE_KEYS.LIBRARY     },
      { name: 'exit_house',  x: T * 18, y: worldW / 2 - T * 2,  w: T * 4,  h: T * 3, target: SCENE_KEYS.SOUTH_HOUSE },
    ];
    ...
    exits.forEach(exit => {
      ...
      this.physics.add.overlap(this.player, zone, () => {
        if (this.playerBlocked) return;
        this.gm.switchMap(exit.target as any);
      });
    });
  }
  ...
  private applyRainWatering(): void {
    ...
    console.log('[SouthYardScene] 비로 인한 자동 물주기 완료');
  }
