...
export class NorthHouseScene extends Phaser.Scene {
  ...
  constructor() {
    super({ key: SCENE_KEYS.NORTH_HOUSE });
  }
  ...
  create(data?: { from?: string }): void {
    ...
    console.log('[NorthHouseScene] 생성 완료');
  }
  ...
  private buildBackground(): void {
    ...
    // 씬 라벨
    this.add.text(W / 2, TILE_SIZE + 4, '북쪽 집', {
  ...
  private createExitZone(): void {
    ...
    this.physics.add.overlap(this.player, zone, () => {
      ...
      this.gm.switchMap(SCENE_KEYS.NORTH_YARD, { from: 'north_house' });
    });
  }
  ...
  private async executeSleep(): Promise<void> {
    ...
    if (!saveResult.success) {
      console.warn('[NorthHouseScene] 저장 실패:', saveResult.reason);
      // HUD 토스트 알림
    ...
    console.log('[NorthHouseScene] 취침 완료');
  }
  ...
  wake(data?: { from?: string }): void {
    this.isSleeping    = false;
    this.playerBlocked = false;
    this.player.setAlpha(1);
    const { x, y } = this.getSpawnPos(data?.from);
